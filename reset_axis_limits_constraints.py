...
with dpg.window() as wnd:
    ...
    with dpg.plot(label="Regular plot", width=-1, height=-50, tag="plot"):
        ...
        dpg.set_axis_limits_constraints(x_axis, -math.inf, 5)

    dpg.add_button(label="Reset limits on X", callback=lambda: dpg.reset_axis_limits_constraints(x_axis))
    dpg.add_button(label="Reset limits on Y", callback=lambda: dpg.reset_axis_limits_constraints(y_axis))
